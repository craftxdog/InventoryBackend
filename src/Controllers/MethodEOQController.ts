...
export class MethodEOQController {
  static createMethodEOQ = async (req: Request, res: Response) => {
    try {
      ...
      const eoq = Math.round(
        Math.sqrt((2 * demandaAnual * costoPedido) / costoMantenimiento),
      );
  ...
  static getAllMethodsEOQ = async (req: Request, res: Response) => {
    try {
      const methodEOQ = await MethodEOQ.find({
  ...
  static updateMethodEOQById = async (req: Request, res: Response) => {
    try {
      ...
      console.error(
        colors.dim.bold(`Error al actualizar el Método EOQ: ${error}`),
      );
      res.status(500).json({ error: "Error interno del servidor" });
    }
  };
  static deleteMethodEOQById = async (req: Request, res: Response) => {
    try {
      req.methods.methodEOQ = req.methods.methodEOQ.filter(
        (methods: any) => methods.toString() !== req.methodEOQ.id.toString(),
      );
      await Promise.allSettled([req.methodEOQ.deleteOne(), req.methods.save()]);
      res.status(201).send("Método Eliminado Correctamente.");
    } catch (error) {
      console.error(
        colors.dim.bold(`Error al eliminar el Método EOQ: ${error}`),
      );
      res.status(500).json({ error: "Error interno del servidor" });
    }
  };
