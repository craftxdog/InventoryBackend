...
export class MethodsController {
  ...
  static getAllMethods = async (req: Request, res: Response) => {
    try {
      const methods = await Methods.find({project: req.project.id}).populate('project')
  ...
  static getMethodById = async (req: Request, res: Response) => {
    try {
      ...
      const methods = await Methods.findById(methodId).populate('methodEOQ').populate('methodCMC')
  ...
  static deleteMethodById = async (req: Request, res: Response) => {
    try {
      ...
      req.project.methods = req.project.methods.filter((method: any) => method.toString() !== req.methods.id.toString())
